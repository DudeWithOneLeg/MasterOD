    ...
    <div className={`flex h-[${isMobile ? '10' : '5'}%] bg-blue-400`}>
      ...
      <div className={`bg-zinc-600 flex flex-row items-center justify-content-between w-full p-${isMobile ? '1 text-sm' : '2'} h-fit overflow-hidden`}>
        <div className="w-full flex flex-row justify-content-between h-fit bg-slate-100 p-1 rounded overflow-hidden">
          <input className="w-full rounded overflow-scroll no-scrollbar" value={preview.includes('https://docs.google.com/gview?embedded=true&url=') ? preview.split('https://docs.google.com/gview?embedded=true&url=')[1] : preview} disabled/>
          {copied ? (
            <p>Copied!</p>
          ) : (
            <img
