...
function Navigation({ isLoaded, setHide }) {
        ...
        <div className="h-[5%] w-screen fixed flex p-2 border-b flex flex-row items-center justify-between text-white">
            ...
            {sessionUser ? (
                <div className="flex justify-items-center">
                    <div className="flex flex-col items-end  cursor-pointer" onClick={() => setShowMenu(!showMenu)}>
                        <img
                            src={profileImg}
                            className="rounded-full h-10"
                            alt="profile"
                        />
                    {showMenu ? <div className="flex flex-col fixed h-fit bg-zinc-950 rounded w-32 mt-12 p-2 border">
                        <div
