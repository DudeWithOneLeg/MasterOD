...
export default function QueryRow({ query, setString, setQuery }) {
    ...
    return (
        <div
        className="flex flex-row grid grid-cols-9 divide divide-x justify-between w-full p-1 hover:bg-slate-500"
        ...
        <div className={`col-span-2 flex flex-${isMobile ? 'col' : 'row'} items-center justify-center`}>
            <p>{date}</p>
            <p>{time}</p>
        </div>
        <div
